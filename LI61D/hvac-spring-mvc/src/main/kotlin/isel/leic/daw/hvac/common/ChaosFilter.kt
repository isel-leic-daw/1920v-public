...
import isel.leic.daw.hvac.BuildInfo
...
class ChaosFilter(private val env: Environment) : Filter {
    ...
    override fun doFilter(request: ServletRequest?, response: ServletResponse?, chain: FilterChain?) {
        ...
        fun isAPIRequest(request: HttpServletRequest) = request.method.toLowerCase() != "options"
        val httpRequest = request as HttpServletRequest

        val isE2ETests = (env.getProperty("api.e2e.tests") ?: "false").toBoolean()
        if (BuildInfo.isDebugBuild() && isAPIRequest(httpRequest) && !isE2ETests) {
